module github.com/mdevilliers/fastly-cli

go 1.12

require (
	github.com/ajg/form v1.5.1 // indirect
	github.com/alecthomas/units v0.0.0-20151022065526-2efee857e7cf // indirect
	github.com/dnaeon/go-vcr v1.0.1 // indirect
	github.com/fastly/go-fastly v1.0.0
	github.com/google/go-querystring v1.0.0
	github.com/google/jsonapi v0.0.0-20181016150055-d0428f63eb51 // indirect
	github.com/hashicorp/go-cleanhttp v0.5.1 // indirect
	github.com/kelseyhightower/envconfig v1.4.0
	github.com/kylelemons/godebug v1.1.0 // indirect
	github.com/manifoldco/promptui v0.3.2
	github.com/mitchellh/mapstructure v1.1.2
	github.com/nicksnyder/go-i18n/v2 v2.0.2 // indirect
	github.com/pelletier/go-toml v1.4.0 // indirect
	github.com/pkg/errors v0.8.1
	github.com/r3labs/diff v0.0.0-20190624005013-fbe9de54bde7
	github.com/sahilm/fuzzy v0.1.0
	github.com/skratchdot/open-golang v0.0.0-20190402232053-79abb63cd66e
	github.com/spf13/cobra v0.0.5
	github.com/stretchr/testify v1.3.0
	golang.org/x/sys v0.0.0-20190617070334-15dcb6c0061f // indirect
	golang.org/x/tools v0.0.0-20190617003114-5aca471b1d59 // indirect
	gopkg.in/alecthomas/kingpin.v3-unstable v3.0.0-20180810215634-df19058c872c // indirect
)
